...
router.post('/login', async (req, res) => {
  try {
    ...
    if (!admin) {
      console.log('Admin not found for username:', username); // Debug log
    ...
    console.log('Admin found, comparing password...'); // Debug log

    // Compare provided password with hashed password using bcrypt.compare
    const isPasswordValid = await admin.comparePassword(password);

    if (!isPasswordValid) {
      console.log('Password comparison failed for username:', username); // Debug log
      return res.status(401).json({
        success: false,
        message: 'Invalid credentials.'
      });
    ...
    console.log('Password comparison successful for username:', username); // Debug log
